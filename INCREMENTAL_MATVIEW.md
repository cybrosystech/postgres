...
- **NULL group keys are excluded automatically; source writes are never blocked.** A NULL in a GROUP BY/DISTINCT key can't be maintained incrementally (the unique index / `ON CONFLICT` and `=` predicates treat NULLs as distinct). So at `CREATE`, the engine injects `<key> IS NOT NULL` into the matview's stored query for every *nullable* key (a `NOTICE` discloses which). NULL-key rows then fall outside the matview's scope: the source `INSERT`/`UPDATE` always succeeds, the matview simply doesn't represent those rows, and it stays **consistent with a full `REFRESH`** (the filter lives in the stored query, so initial population, REFRESH, and deltas all agree). `NOT NULL` keys get no filter. The matview therefore has no "NULL/unknown" group — the sensible default for a grouped report. Note this **differs from a normal (non-incremental) matview**, which keeps the NULL group; the incremental matview is still self-consistent (it equals its own `REFRESH`). Full NULL-group fidelity (`NULLS NOT DISTINCT` index + `IS NOT DISTINCT FROM`), so incremental matches a normal matview, is a Phase 2 deliverable.
- **Concurrency-uncertified shapes warn at `CREATE`**: outer joins, self-joins, and `UNION ALL` pass single-threaded correctness but are not yet covered by the concurrent-write battery, so `CREATE` emits a `WARNING` advising validation before production reliance. They are not blocked.
