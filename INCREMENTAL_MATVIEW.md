...
- **Group-key columns must not contain NULL values** (data requirement, not enforced): the dedup key uses a unique index / `ON CONFLICT` and `=` join predicates, both of which treat NULLs as distinct — so an *actual* NULL value in a GROUP BY (or DISTINCT) column would produce duplicate/incorrect rows. A column being *nullable by schema* is fine as long as its data has no NULLs (the common case). Group on `NOT NULL` columns, or filter NULL keys out in `WHERE`. (Full NULL-key support — `NULLS NOT DISTINCT` index + `IS NOT DISTINCT FROM` joins — is possible but deferred.)
