...
- **Group-key columns must not contain NULL values** (enforced by a runtime guard): a NULL in a GROUP BY/DISTINCT key cannot be maintained incrementally (the unique index / `ON CONFLICT` and `=` predicates treat NULLs as distinct). Rather than corrupt silently, an insert delta that would introduce a NULL key **raises a clear error** — so you get a loud failure, never wrong numbers. Because the guard runs in the source `INSERT`'s transaction, that error **fails the source write**: while the matview exists, the grouped column effectively acts `NOT NULL` for the source table. A column being *nullable by schema* is fine as long as its data has no NULLs (the common case, paid for with a cheap post-insert check); `NOT NULL` keys pay nothing. Group on `NOT NULL` columns, or — to keep writing NULLs to the source — add `WHERE <col> IS NOT NULL` to the matview so it ignores NULL-key rows (verified). (Full NULL-key *support* — `NULLS NOT DISTINCT` index + `IS NOT DISTINCT FROM` joins — is deferred to the delta-core redesign.)
