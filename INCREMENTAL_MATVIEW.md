...
- **Group-key columns must not contain NULL values** (enforced by a runtime guard): a NULL in a GROUP BY/DISTINCT key cannot be maintained incrementally (the unique index / `ON CONFLICT` and `=` predicates treat NULLs as distinct). Rather than corrupt silently, an insert delta that would introduce a NULL key **raises a clear error and rolls back** — so you get a loud failure, never wrong numbers. A column being *nullable by schema* is fine as long as its data has no NULLs (the common case, paid for with a cheap post-insert check); `NOT NULL` keys pay nothing. Group on `NOT NULL` columns, or filter NULL keys out in `WHERE`. (Full NULL-key *support* — `NULLS NOT DISTINCT` index + `IS NOT DISTINCT FROM` joins — is deferred to the delta-core redesign.)
