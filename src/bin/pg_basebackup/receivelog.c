				  ...
				  XLogRecPtr *blockpos)
{
	...
	*blockpos = fe_recvint64(&copybuf[1]);
	...
	while (bytes_left)
	{
		...
		/*
		 * The server hands us plaintext WAL, so re-encrypt it in place before
		 * it reaches disk; otherwise the backup would hold plaintext WAL that
		 * the restored cluster would then try to decrypt.
		 */
		if (stream->encrypt)
		{
