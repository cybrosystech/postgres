							  ...
							  bool streaming_wal)
{
	...
	*((const astreamer_ops **) &streamer->base.bbs_ops) =
	...
	streamer->streaming_wal = streaming_wal;
	...
	switch (context)
	{
		case ASTREAMER_MEMBER_HEADER:
			...
			if (strcmp(member->pathname, "pg_tde/wal_keys") == 0)
			{
				...
				else if (mystreamer->streaming_wal)
				{
					/*
					 * Only meaningful when WAL is streamed: -X fetch copies
					 * the segments verbatim, so they stay encrypted, and -X
					 * none ships no WAL at all.  Warning in those cases would
					 * be advising a fix (-E) that is rejected for them.
					 */
