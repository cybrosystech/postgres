...
/*
 * True once --keyring-path has installed the TDE xlog storage manager, i.e.
 * the WAL we are about to read is encrypted.  Without it we take the stock
 * code paths unchanged.
 */
static bool decrypt_wal = false;
...
search_directory(const char *directory, const char *fname, int *WalSegSz)
{
	...
	if (fd >= 0)
	{
		...
		if (decrypt_wal)
		{
			...
			/*
			 * We cannot take WalSegSz from the first page header, because that
			 * header is itself encrypted and the segment size is required to
			 * derive the decryption offset.  Use the file's actual size
			 * instead; xlp_seg_size is only a cross-check anyway.
			 */
			fsize = lseek(fd, 0, SEEK_END);
			if (fsize < 0)
				pg_fatal("could not seek in file \"%s\": %m", fname);

			if (!IsValidWalSegSize(fsize))
			{
				pg_log_error(ngettext("invalid WAL segment size in WAL file \"%s\" (%lld byte)",
									  "invalid WAL segment size in WAL file \"%s\" (%lld bytes)",
									  (int) fsize),
			...
			r = xlog_smgr->seg_read(fd, buf.data, XLOG_BLCKSZ, 0,
									tli, segno, fsize);
			if (r == XLOG_BLCKSZ)
			{
				*WalSegSz = fsize;
...
usage(void)
{
	...
	printf(_("  -k, --keyring-path=PATH\n"
			 "                         directory holding the WAL keyring config files\n"
			 "                         (wal_keys and 1664_providers, usually $PGDATA/pg_tde);\n"
			 "                         WAL is not decrypted if this is not set\n"));
...
main(int argc, char **argv)
{
	...
	static struct option long_options[] = {
		...
		{"keyring-path", required_argument, NULL, 'k'},
	...
	/*
	 * Install the TDE xlog storage manager so encrypted WAL can be read.
	 * Skipped entirely when --keyring-path was not given, leaving the stock
	 * read path in place.
	 */
	if (kringdir != NULL)
	{
