...
/*
 * The backend headers included below (xloginsert.h, bufmgr.h, guc.h,
 * memutils.h) need the backend type definitions -- Datum, MemoryContext and
 * friends -- which postgres_fe.h does not provide, so pull postgres.h in even
 * for a frontend build.  This used to arrive implicitly via
 * access/xlog_smgr.h back when that header included postgres.h itself.
 */
#include "postgres.h"
#else
#include "postgres.h"
