 ...
 *
 * Note: unlike the backend, frontend tools deliberately do not register the
 * KMIP provider.  Doing so would pull the vendored libkmip C++ library (and a
 * C++ runtime) into every client binary for a provider that a client is very
 * unlikely to need.  A cluster whose principal key lives in a KMIP server will
 * therefore report an unknown provider type here rather than silently
 * misbehaving; the server itself retains full KMIP support.
 */
void
