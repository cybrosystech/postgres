...
typedef struct RI_ConstraintInfo
{
	...
	bool		conindid_is_exact;	/* conindid has exactly nkeys key columns
									 * (see DBblue note in
...
ri_LoadConstraintInfo(Oid constraintOid)
{
	 ...
	 * that holds.  Cached here rather than tested per row: indnkeyatts cannot
	 * change for a live index, and replacing the index rewrites
...
ri_fastpath_is_applicable(const RI_ConstraintInfo *riinfo)
{
	 ...
	 * keys positionally.  transformFkeyCheckAttrs() can accept a wider unique
	 * index when the referenced table is partitioned, and although such a
	 * constraint cannot reach this point today (pk_is_partitioned is rejected
	 * above, and leaf-level clones carry action triggers only), checking it
	 * keeps a future change to those conditions degrading to the SPI path
	 * rather than probing the wrong index column.  Computed once in
	 * ri_LoadConstraintInfo(), not per row.
	 */
