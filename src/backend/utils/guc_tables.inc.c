...
struct config_generic ConfigureNames[] =
{
	...
	{
		.name = "dbblue_count_cache",
		.context = PGC_USERSET,
		.group = QUERY_TUNING_METHOD,
		/* translator: GUC parameter "dbblue_count_cache" short description */
		.short_desc = gettext_noop("Enables the DBblue session-local COUNT(*) result cache."),
		.flags = GUC_EXPLAIN,
		.vartype = PGC_BOOL,
		._bool = {
			.variable = &dbblue_count_cache,
			.boot_val = true,
		},
	},

	{
		.name = "dbblue_count_cache_ttl",
		.context = PGC_USERSET,
		.group = QUERY_TUNING_METHOD,
		/* translator: GUC parameter "dbblue_count_cache_ttl" short description */
		.short_desc = gettext_noop("Sets the lifetime (in seconds) of DBblue COUNT cache entries."),
		.flags = GUC_UNIT_S,
		.vartype = PGC_INT,
		._int = {
			.variable = &dbblue_countcache_ttl,
			.boot_val = 300,
			.min = 1,
			.max = 86400,
		},
	...
	{
		.name = "dbblue_offset_flip",
		.context = PGC_USERSET,
		.group = QUERY_TUNING_METHOD,
		/* translator: GUC parameter "dbblue_offset_flip" short description */
		.short_desc = gettext_noop("Enables the DBblue OFFSET-flip planner rewrite (requires dbblue_count_cache)."),
		.flags = GUC_EXPLAIN,
		.vartype = PGC_BOOL,
		._bool = {
			.variable = &dbblue_offset_flip,
			.boot_val = true,
		},
	},
...

	{
		...
		.long_desc = gettext_noop("-1 disables the timeout"),
		.flags = GUC_UNIT_MS,
		.vartype = PGC_INT,
		._int = {
