/* contrib/dbblue_columnar/dbblue_columnar--1.4.sql */
...
-- plus a way to see it directly if they ever do. See the 1.3--1.4 delta for
-- the full rationale.
-- ---------------------------------------------------------------------------
...
-- A fresh install of the extension means dbblue_columnar_relations is about to
-- be created empty (see below). Anything already resident in the store for
-- this database at THIS instant is therefore, unconditionally, an orphan from
-- before this create - nothing could have legitimately built it through an
-- extension that does not exist yet. Reclaim it now, so a fresh CREATE
-- EXTENSION gives the same guarantee a server restart already gives: the
-- store starts empty. Deliberately only here, in the fresh-install script -
-- never in an --X--Y.sql upgrade delta, which must preserve a live install's
-- real stores.
SELECT dbblue_columnar_reset_database();
