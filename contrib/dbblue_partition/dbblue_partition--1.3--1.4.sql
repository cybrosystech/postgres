/* contrib/dbblue_partition/dbblue_partition--1.3--1.4.sql */

-- complain if script is sourced in psql, rather than via ALTER EXTENSION
\echo Use "ALTER EXTENSION dbblue_partition UPDATE TO '1.4'" to load this file. \quit

/*
 * 1.4: changes driven by testing on a 3.3M-row Odoo sale_order.
 *
 * - dbblue_partition_model() now defaults to p_interval => '1 year' and
 *   p_single_transaction => true.  Odoo tables span years, and an atomic
 *   conversion means an interrupted run leaves the database exactly as it
 *   was, so Odoo keeps working.  Pass p_single_transaction => false for a
 *   very large table, where holding ACCESS EXCLUSIVE for the whole copy
 *   costs more than a resumable, partially-visible migration.
 * - dbblue_partition_undo() reconciles column drift by itself when the
 *   backup is empty (adding or dropping a column on an empty table loses
 *   nothing), so a module update between conversion and undo is no longer
 *   an obstacle.  It still refuses when the backup holds rows.
 * - dbblue_partition_undo() re-validates incoming foreign keys that were
 *   valid before the conversion; undoing an *interrupted* conversion used
 *   to leave them NOT VALID permanently.
 * - Materialized views are now captured and recreated -- with their
 *   indexes, owner, comment, grants and populated state -- instead of
 *   blocking the conversion.
 * - dbblue_partition_status() reports total_size (a partitioned parent's
 *   own size is always 0, which reads as "no data") and
 *   rows_not_visible_to_odoo for an unfinished migration.
 * - A control column with a precision modifier (timestamp(3)) is no longer
 *   falsely rejected.
 * - Adds dbblue_partition_odoo_compat_check(), which was appended to the
 *   1.3 script after 1.3 had already been applied somewhere.
 */
...
/*
 * The status function gains two output columns, and a RETURNS TABLE
 * signature cannot be widened by CREATE OR REPLACE, so drop it first.
 * Dropping an extension member from inside the extension's own update
 * script is permitted.
 */
/*
 * A materialized view has to be refreshed *after* the data migration, not
 * during the structural swap where the new table is still empty.  Record
 * which ones need it so the model procedure can refresh them at the end.
 */
ALTER TABLE @extschema@.dbblue_partition_catalog
	ADD COLUMN IF NOT EXISTS matviews_to_refresh jsonb NOT NULL DEFAULT '[]'::jsonb;

DROP FUNCTION @extschema@.dbblue_partition_status(text, text);
...
CREATE OR REPLACE FUNCTION @extschema@.dbblue_partition_odoo_compat_check(
